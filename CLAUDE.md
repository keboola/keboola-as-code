...
This is a **Go 1.25+ monorepo** providing infrastructure-as-code tooling for the Keboola data platform. The project consists of three main services:

1. **CLI (kbc)** - Bidirectional sync between local directory and Keboola projects
2. **Templates API** - Create and apply templates using Jsonnet
3. **Stream Service** - Real-time data ingestion proxy with batch imports to Storage tables
4. **Apps Proxy** - Application proxying and routing (secondary component)
...
- `cmd/` - Service entry points (main.go files)
  - `kbc/` - CLI binary
  - `templates-api/` - Templates API service
  - `stream/` - Stream service
  - `apps-proxy/` - Apps Proxy service

### Service Implementations
- `internal/pkg/service/` - Service implementations
  - `cli/` - CLI commands (Cobra), dialogs, operations
  - `templates/` - Templates API logic
  - `stream/` - Stream service (API nodes + Worker nodes)
  - `appsproxy/` - Apps Proxy logic
  - `common/` - Shared infrastructure
    - `dependencies/` - DI containers
    - `etcdclient/` - etcd client wrapper
    - `etcdop/` - etcd operations framework
    - `httpserver/` - HTTP server components
    - `servicectx/` - Process lifecycle management
    - `task/` - Distributed task execution

### Core Logic
- `internal/pkg/model/` - Domain models (Branch, Config, ConfigRow, SharedCode, Variables)
- `internal/pkg/state/` - State synchronization between local and remote
- `internal/pkg/mapper/` - Object mapping between representations
- `internal/pkg/encoding/` - JSON/Jsonnet handling
- `internal/pkg/template/` - Template processing
- `internal/pkg/telemetry/` - OpenTelemetry integration
- `pkg/lib/operation/` - Reusable operations (public library code)

### API & Testing
- `api/` - Goa API design files (DSL specifications)
- `test/` - E2E and integration tests organized by service

### Infrastructure
- `provisioning/` - Kubernetes manifests and deployment configs (dev/docker only; production is GitOps)
...
- **Prometheus** metrics export (when running with Docker Compose)
- **PProf** profiling support
- Structured logging with **zerolog** via `internal/pkg/log`
- Access Prometheus UI at `localhost:9090` when running services with `docker compose up -d`

## Component Interactions

```
Keboola Storage API (Cloud)
         ↕ HTTP/REST
CLI + Templates API + Stream Service
         ↕
   etcd Cluster
(distributed storage/coordination)
```

- **CLI → Storage API**: Direct HTTP calls for pull/push
- **Templates API → etcd**: Locks for atomic operations
- **Stream Service → etcd**: Stores streaming data until batch import
- **All Services → OpenTelemetry**: Distributed tracing & metrics
