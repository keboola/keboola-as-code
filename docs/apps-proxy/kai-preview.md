...
```bash
KBC_APPS_PROXY_KAI_PREVIEW_HANDSHAKE_SIGNING_KEY="${HANDSHAKE_KEY}" \
KBC_APPS_PROXY_KAI_PREVIEW_SESSION_SIGNING_KEY="${SESSION_KEY}" \
KBC_APPS_PROXY_KAI_PREVIEW_SESSION_TTL="4h" \
KBC_APPS_PROXY_KAI_PREVIEW_ALLOWED_IDE_ORIGINS="${IDE_ORIGIN}" \
KBC_APPS_PROXY_STORAGE_API_URL="https://connection.keboola.com" \
  ./apps-proxy
```
