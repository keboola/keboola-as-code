...
func CreateBranchRequest(branch *model.Branch) Request[*model.Job] {
	return createBranchRequest(branch).
		OnSuccess(func(sender Sender, response HttpResponse[*model.Job]) error {
			// Wait for storage job and set branch ID.
			job := response.Result()
			if err := waitForJob(sender, job); err == nil {
...
func DeleteBranchRequest(key model.BranchKey) Request[*model.Job] {
	return deleteBranchRequest(key).
		OnSuccess(func(sender Sender, response HttpResponse[*model.Job]) error {
			// Wait for storage job
			job := response.Result()
			return waitForJob(sender, job)
		})
}
