...
func (a *Api) ListComponentsRequest(branchId model.BranchId) *http.Request {
	components := make([]*model.ComponentWithConfigs, 0)
	return a.
		NewRequest(http.MethodGet, "branch/{branchId}/components").
...
func (a *Api) ListConfigMetadataRequest(branchId model.BranchId) *http.Request {
	metadata := ConfigMetadataResponse{}
	return a.
		NewRequest(http.MethodGet, "branch/{branchId}/search/component-configurations").
		SetPathParam("branchId", branchId.String()).
...
func (a *Api) AppendConfigMetadataRequest(config *model.Config) *http.Request {
	...
	return a.
		NewRequest(http.MethodPost, "branch/{branchId}/components/{componentId}/configs/{configId}/metadata").
...
func (a *Api) GetConfigRequest(branchId model.BranchId, componentId model.ComponentId, configId model.ConfigId) *http.Request {
	config := &model.Config{}
	config.BranchId = branchId
	config.ComponentId = componentId
	return a.
		NewRequest(http.MethodGet, "branch/{branchId}/components/{componentId}/configs/{configId}").
		SetPathParam("branchId", branchId.String()).
...
func (a *Api) CreateConfigRequest(config *model.ConfigWithRows) (*http.Request, error) {
	// Data
	...
	configRequest = a.
		NewRequest(http.MethodPost, "branch/{branchId}/components/{componentId}/configs").
		SetPathParam("branchId", config.BranchId.String()).
		SetPathParam("componentId", config.ComponentId.String()).
		SetFormBody(values).
		SetResult(config).
...
func (a *Api) UpdateConfigRequest(config *model.Config, changed model.ChangedFields) (*http.Request, error) {
	// Id is required
	if config.Id == "" {
	...
	// Update config
	request := a.
		NewRequest(http.MethodPut, "branch/{branchId}/components/{componentId}/configs/{configId}").
		SetPathParam("branchId", config.BranchId.String()).
		SetPathParam("componentId", config.ComponentId.String()).
		SetPathParam("configId", config.Id.String()).
...
func (a *Api) DeleteConfigRequest(key model.ConfigKey) *http.Request {
	return a.NewRequest(http.MethodDelete, "branch/{branchId}/components/{componentId}/configs/{configId}").
...
func (a *Api) DeleteConfigsInBranchRequest(key model.BranchKey) *http.Request {
