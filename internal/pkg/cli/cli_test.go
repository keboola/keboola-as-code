...
func newTestRootCommand() (*rootCommand, *utils.Writer) {
	...
	return NewRootCommand(in, out, out, interaction.NewPrompt(in, out, out), env.Empty(), aferofs.NewMemoryFs), out
}

func newTestRootCommandWithTty(tty *os.File) *rootCommand {
	prompt := interaction.NewPrompt(tty, tty, tty)
	prompt.Interactive = true
	return NewRootCommand(tty, tty, tty, prompt, env.Empty(), aferofs.NewMemoryFs)
}
