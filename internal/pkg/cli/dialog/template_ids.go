...
func (d *templateIdsDialog) parse(result string) ([]template.ConfigDef, error) {
	...
	for scanner.Scan() {
		...
		case strings.HasPrefix(line, `## Config`):
			...
			if m == nil {
				errs.Append(fmt.Errorf(`line %d: cannot parse "%s"`, lineNum, line))
				continue
			}
		...
		// Parse template ID
		if !scanner.Scan() {
			errs.Append(fmt.Errorf(`expected line, found EOF`))
			continue
		}
		lineNum++
		...
		switch {
		case len(id) == 0:
			errs.Append(fmt.Errorf(`line %d: unexpected empty line`, lineNum))
			continue
		case ids[id]:
			errs.Append(fmt.Errorf(`line %d: duplicate ID "%s"`, lineNum, id))
			continue
		default:
	...
	for _, c := range d.configs {
		...
		if len(id) == 0 {
			errs.Append(fmt.Errorf(`missing ID for %s`, c.String()))
			continue
		}
		...
		for _, r := range c.Rows {
			...
			if len(id) == 0 {
				errs.Append(fmt.Errorf(`missing ID for %s`, r.String()))
				continue
			}
...
`
	...
	for _, c := range d.configs {
		lines.WriteString(fmt.Sprintf("## Config \"%s\" %s:%s\n%s\n\n", c.Name, c.ComponentId, c.ConfigId, idByKey[c.Key().String()]))
		for _, r := range c.Rows {
			lines.WriteString(fmt.Sprintf("### Row \"%s\" %s:%s:%s\n%s\n\n", r.Name, r.ComponentId, r.ConfigId, r.ConfigRowId, idByKey[r.Key().String()]))
		}
	}
