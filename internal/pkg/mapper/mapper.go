...
type LocalFileLoadMapper interface {
	LoadLocalFile(def *filesystem.FileDef, fileType filesystem.FileType, next fileloader.Handler) (filesystem.File, error)
}
...
func (m *Mapper) NewFileLoader(defaultHandler fileloader.Handler) filesystem.FileLoader {
	return fileloader.New(func(def *filesystem.FileDef, fileType filesystem.FileType) (filesystem.File, error) {
		return m.LoadLocalFile(def, fileType, defaultHandler)
	})
}
...
func (m *Mapper) LoadLocalFile(def *filesystem.FileDef, fileType filesystem.FileType, handler fileloader.Handler) (filesystem.File, error) {
	// Generate handlers chain, eg.  mapper1(mapper2(mapper3(root())))
