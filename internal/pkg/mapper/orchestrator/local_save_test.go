...
func TestMapBeforeLocalSave(t *testing.T) {
	...
	// Trim spaces in generated files
	for _, file := range recipe.ExtraFiles {
		file.Content = regexp.MustCompile(`\s`).ReplaceAllString(file.Content, ``)
	}
	...
	assert.Equal(t, []*filesystem.File{
		...
		filesystem.
			CreateFile(
				...
				`{"name":"Task1","task":{"mode":"run","configPath":"extractor/target-config-1"},"continueOnFailure":false,"enabled":true}`,
			).
			SetDescription(`task config file`),
		filesystem.
			CreateFile(
				phasesDir+`/001-phase/tasks/002-task-2/task.json`,
				`{"name":"Task2","task":{"mode":"run","configPath":"extractor/target-config-2"},"continueOnFailure":false,"enabled":false}`,
			).
			SetDescription(`task config file`),
		filesystem.
			CreateFile(
				phasesDir+`/002-phase-with-deps/phase.json`,
				`{"name":"PhaseWithDeps","dependsOn":["phases/001-phase"]}`,
			).
			SetDescription(`phase config file`),
		filesystem.
			CreateFile(
				phasesDir+`/002-phase-with-deps/tasks/001-task-3/task.json`,
				`{"name":"Task3","task":{"mode":"run","configPath":"extractor/target-config-3"},"continueOnFailure":false,"enabled":true}`,
			).
			SetDescription(`task config file`),
	}, recipe.ExtraFiles)
}
