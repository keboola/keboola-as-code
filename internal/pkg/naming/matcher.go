...
func (m PathMatcher) MatchConfigPath(parentKey Key, path AbsPath) (componentId ComponentId, err error) {
	...
	if parent.IsBranch() {
		// Shared code
		if matched, _ := m.template.SharedCodeConfig.MatchPath(path.RelativePath); matched {
			return SharedCodeComponentId, nil
		}

		// Ordinary config
		if matched, matches := m.template.Config.MatchPath(path.RelativePath); matched {
			// Get component ID
	...
	if parent.IsConfig() {
		// Variables
		if matched, _ := m.template.VariablesConfig.MatchPath(path.RelativePath); matched {
			return VariablesComponentId, nil
		}
		// Scheduler
		if matched, _ := m.template.SchedulerConfig.MatchPath(path.RelativePath); matched {
...
func (m PathMatcher) MatchConfigRowPath(component *Component, path AbsPath) bool {
	...
	if component.IsSharedCode() {
		matched, _ := m.template.SharedCodeConfigRow.MatchPath(path.RelativePath)
		return matched
	}

	// Variables
	if component.IsVariables() {
		matched, _ := m.template.VariablesValuesRow.MatchPath(path.RelativePath)
		return matched
	}

	// Ordinary config row
	matched, _ := m.template.ConfigRow.MatchPath(path.RelativePath)
	return matched
}
