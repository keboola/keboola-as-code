...
func customMutationTemplate() *gen.Template {
	// Create temp dir
	tmpDir, err := ioutil.TempDir(os.TempDir(), "keboola-model-compiler")
	if err != nil {
		panic(err)
	}
	defer func() {
	...
	if err := os.WriteFile(filepath.Join(tmpDir, "mutation.tmpl"), originalData, 0600); err != nil {
		panic(err)
	}

	// Load patch
	patchData := mutationTemplatePatch()
	if err := os.WriteFile(filepath.Join(tmpDir, "mutation.tmpl.patch"), patchData, 0600); err != nil {
		panic(err)
	}
