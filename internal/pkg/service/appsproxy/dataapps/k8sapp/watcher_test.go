...
func newAppObjectWithServiceRef(k8sName, appID string, state k8sapp.AppActualState, serviceRef string) *unstructured.Unstructured {
	obj := newAppObject(k8sName, appID, state)
	obj.Object["status"].(map[string]any)["appsProxyServiceRef"] = serviceRef
	return obj
}
...
func TestStateWatcher_GetState_UpstreamTarget(t *testing.T) {
	...
	appObj := newAppObjectWithServiceRef("my-app-k8s", "app-123", k8sapp.AppActualStateRunning, "http://svc-name.keboola.svc.cluster.local:8080")
	_, err := fakeClient.Resource(k8sapp.AppGVR).Namespace(testNamespace).Create(
		t.Context(), appObj, metav1.CreateOptions{},
	)
	require.NoError(t, err)
	...
	assert.Equal(t, "svc-name.keboola.svc.cluster.local:8080", info.UpstreamTarget.Host)
}
