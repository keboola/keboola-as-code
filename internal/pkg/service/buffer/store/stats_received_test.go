...
func TestStore_GetReceivedStatsByFile(t *testing.T) {
	...
	assert.NoError(t, store.UpdateSliceReceivedStats(ctx, "node1", []model.SliceStats{
		{
			...
			Stats:    model.Stats{Count: 10, Size: 100, LastAt: key.UTCTime(time1.Add(1 * time.Hour))},
		},
		{
			SliceKey: sliceF1S2Key,
			Stats:    model.Stats{Count: 1, Size: 10, LastAt: key.UTCTime(time1.Add(1 * time.Hour))},
		},
		{
			SliceKey: sliceF2S1Key,
			Stats:    model.Stats{Count: 2, Size: 20, LastAt: key.UTCTime(time2.Add(1 * time.Hour))},
		},
	}))
	...
	assert.NoError(t, store.UpdateSliceReceivedStats(ctx, "node1", []model.SliceStats{
		{
			SliceKey: sliceF1S1Key,
			Stats:    model.Stats{Count: 30, Size: 300, LastAt: key.UTCTime(time1.Add(2 * time.Hour))},
		},
		{
			SliceKey: sliceF1S2Key,
			Stats:    model.Stats{Count: 4, Size: 40, LastAt: key.UTCTime(time1.Add(2 * time.Hour))},
		},
		{
			SliceKey: sliceF2S1Key,
			Stats:    model.Stats{Count: 5, Size: 50, LastAt: key.UTCTime(time2.Add(2 * time.Hour))},
		},
	}))
	// node1[sliceF1S1Key] + node1[sliceF1S2Key]
	...
	assert.NoError(t, store.UpdateSliceReceivedStats(ctx, "node2", []model.SliceStats{
		{
			SliceKey: sliceF1S1Key,
			Stats:    model.Stats{Count: 60, Size: 600, LastAt: key.UTCTime(time1.Add(3 * time.Hour))},
		},
		{
			SliceKey: sliceF1S2Key,
			Stats:    model.Stats{Count: 7, Size: 70, LastAt: key.UTCTime(time1.Add(3 * time.Hour))},
		},
		{
			SliceKey: sliceF2S1Key,
			Stats:    model.Stats{Count: 8, Size: 80, LastAt: key.UTCTime(time2.Add(3 * time.Hour))},
		},
	}))
	...
	assert.NoError(t, store.UpdateSliceReceivedStats(ctx, "node2", []model.SliceStats{
		{
			SliceKey: sliceF1S1Key,
			Stats:    model.Stats{Count: 10, Size: 100, LastAt: key.UTCTime(time1.Add(4 * time.Hour))},
		},
		{
			SliceKey: sliceF1S2Key,
			Stats:    model.Stats{Count: 2, Size: 20, LastAt: key.UTCTime(time1.Add(4 * time.Hour))},
		},
		{
			SliceKey: sliceF2S1Key,
			Stats:    model.Stats{Count: 3, Size: 30, LastAt: key.UTCTime(time2.Add(4 * time.Hour))},
		},
	}))
	// node1[sliceF1S1Key] + node1[sliceF1S2Key] + node2[sliceF1S1Key] + node2[sliceF1S2Key]
...
func TestStore_GetReceivedStatsByFile_Many(t *testing.T) {
		...
		for s := 1; s <= 1000; s++ {
			...
			stats = append(stats, model.SliceStats{
				...
				Stats: model.Stats{
					Count:  1,
					Size:   10,
					LastAt: key.UTCTime(sliceOpenedAt.Add(1 * time.Minute)),
				},
			})
		}
	...
	assert.Equal(t, key.UTCTime(fileOpenedAt.Add(1000*time.Hour+1*time.Minute)), stats.LastAt)
}
...
func TestStore_GetReceivedStatsBySlice(t *testing.T) {
	...
	assert.NoError(t, store.UpdateSliceReceivedStats(ctx, "node1", []model.SliceStats{
		...
		{
			SliceKey: slice2Key,
			Stats:    model.Stats{Count: 1, Size: 10, LastAt: key.UTCTime(time2.Add(1 * time.Hour))},
		},
	}))
	...
	assert.NoError(t, store.UpdateSliceReceivedStats(ctx, "node1", []model.SliceStats{
		{
			SliceKey: slice1Key,
			Stats:    model.Stats{Count: 20, Size: 200, LastAt: key.UTCTime(time1.Add(2 * time.Hour))},
		},
		{
			SliceKey: slice2Key,
			Stats:    model.Stats{Count: 2, Size: 20, LastAt: key.UTCTime(time2.Add(2 * time.Hour))},
		},
	}))
	...
	assert.NoError(t, store.UpdateSliceReceivedStats(ctx, "node2", []model.SliceStats{
		{
			SliceKey: slice1Key,
			Stats:    model.Stats{Count: 100, Size: 1000, LastAt: key.UTCTime(time1.Add(3 * time.Hour))},
		},
		{
			SliceKey: slice2Key,
			Stats:    model.Stats{Count: 10, Size: 100, LastAt: key.UTCTime(time2.Add(3 * time.Hour))},
		},
	}))
	...
	assert.NoError(t, store.UpdateSliceReceivedStats(ctx, "node2", []model.SliceStats{
		{
			SliceKey: slice1Key,
			Stats:    model.Stats{Count: 200, Size: 2000, LastAt: key.UTCTime(time1.Add(4 * time.Hour))},
		},
		{
			SliceKey: slice2Key,
			Stats:    model.Stats{Count: 20, Size: 200, LastAt: key.UTCTime(time2.Add(4 * time.Hour))},
		},
	}))
...
func TestStore_GetReceivedStatsBySlice_Many(t *testing.T) {
		...
		assert.NoError(t, store.UpdateSliceReceivedStats(ctx, nodeID, []model.SliceStats{
			{
				...
				Stats: model.Stats{
					Count:  1,
					Size:   10,
					LastAt: key.UTCTime(fileOpenedAt.Add(time.Duration(n) * time.Minute)),
				},
			},
		}))
	}
	...
	assert.Equal(t, key.UTCTime(fileOpenedAt.Add(100*time.Minute)), stats.LastAt)
	...
	err := store.UpdateSliceReceivedStats(ctx, "my-node", []model.SliceStats{
		{
			...
			Stats: model.Stats{
				Count:  111,
				Size:   1111,
				LastAt: key.UTCTime(time1.Add(time.Hour * 2)),
			},
		},
		{
			...
			Stats: model.Stats{
				Count:  222,
				Size:   2222,
				LastAt: key.UTCTime(time2.Add(time.Hour * 2)),
			},
		},
	})
	assert.NoError(t, err)
...
-----
{
  ...
  "count": 111,
  "size": 1111,
  "lastAt": "0001-01-01T02:01:00.000Z"
}
>>>>>
...
-----
{
  ...
  "count": 222,
  "size": 2222,
  "lastAt": "0001-01-01T14:01:00.000Z"
}
>>>>>
`)
}
...
func assertFileStats(t *testing.T, store *Store, fileKey key.FileKey, count, size uint64, lastAt time.Time) {
	...
	assert.Equal(t, key.UTCTime(lastAt), stats.LastAt)
}
