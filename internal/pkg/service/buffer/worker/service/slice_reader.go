...
func newRecordsReader(ctx context.Context, logger log.Logger, client *etcd.Client, schema *schema.Schema, slice model.Slice, receivedStats model.Stats, uploadStats *model.UploadStats) io.Reader {
	out, in := io.Pipe()
	go func() {
		var err error
		defer func() {
