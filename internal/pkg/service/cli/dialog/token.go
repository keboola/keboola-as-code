...
func (p *Dialogs) AskStorageAPIToken(d hostAndTokenDependencies) (string, error) {
	opts := d.Options()
	token := opts.GetString(options.StorageAPITokenOpt)
	if len(token) == 0 {
	...
	} else if opts.KeySetBy(options.StorageAPITokenOpt) == cliconfig.SetByEnv {
	...
	opts.Set(options.StorageAPITokenOpt, token)
	return token, nil
}
