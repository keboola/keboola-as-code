...
// ForType is generic type for all typed operations in the package, it implements the Op interface.
...
func (v ForType[R]) WithProcessor(p func(ctx context.Context, result *Result[R])) ForType[R] {
...
func (v ForType[R]) WithResultTo(ptr *R) ForType[R] {
...
func (v ForType[R]) WithResultValidator(fn func(R) error) ForType[R] {
...
func (v ForType[R]) Do(ctx context.Context, opts ...Option) *Result[R] {
	// Create low-level operation
	op, err := v.Op(ctx)
	if err != nil {
...
func (v ForType[R]) mapResponse(ctx context.Context, raw RawResponse) *Result[R] {
	r := newResult[R](&raw)
