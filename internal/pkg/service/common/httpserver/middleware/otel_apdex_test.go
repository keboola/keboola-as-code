...
func TestApdexCounters(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range cases {
		...
		for _, req := range tc.Requests {
			counters.Add(ctx, req.DurationMs, req.StatusCode)
		}

		metrics := tel.Metrics(t)
		assert.Len(t, metrics, 2)
		assert.Len(t, metrics[0].Data.(metricdata.Sum[float64]).DataPoints, 1)
		assert.Len(t, metrics[1].Data.(metricdata.Sum[float64]).DataPoints, 1)
		t500Data := metrics[0].Data.(metricdata.Sum[float64]).DataPoints[0]
		t1000Data := metrics[1].Data.(metricdata.Sum[float64]).DataPoints[0]
		assert.Equal(t, tc.ExpectedApdexT500, t500Data.Value/float64(len(tc.Requests)))
		assert.Equal(t, tc.ExpectedApdexT1000, t1000Data.Value/float64(len(tc.Requests)))
	}
}
