package schema

import (
	"github.com/keboola/keboola-as-code/internal/pkg/service/common/etcdop"
	"github.com/keboola/keboola-as-code/internal/pkg/service/common/etcdop/serde"
	"github.com/keboola/keboola-as-code/internal/pkg/service/stream/definition"
	"github.com/keboola/keboola-as-code/internal/pkg/service/stream/definition/key"
	"github.com/keboola/keboola-as-code/internal/pkg/utils/errors"
)

type (
	// Job is an etcd prefix that stores all Keboola-specific data we need for job polling.
	Job struct {
		etcdop.PrefixT[definition.Job]
	}
	JobInState Job
)

func New(s *serde.Serde) Job {
	return Job{PrefixT: etcdop.NewTypedPrefix[definition.Job]("definition/job", s)}
}

// Active prefix contains all not deleted objects.
func (j Job) Active() JobInState {
	return JobInState{PrefixT: j.PrefixT.Add("active")}
}

// Deleted prefix contains all deleted objects whose parent existed on deleted.
func (j Job) Deleted() JobInState {
	return JobInState{PrefixT: j.PrefixT.Add("deleted")}
}

func (j Job) ForSink(k key.JobKey) etcdop.KeyT[definition.Job] {
	return j.PrefixT.Key(k.String())
}
func (j JobInState) In(objectKey any) etcdop.PrefixT[definition.Job] {
	switch k := objectKey.(type) {
	case key.SinkKey:
		return j.InSink(k)
	default:
		panic(errors.Errorf(`unexpected Job parent key type "%T"`, objectKey))
	}
}

func (j JobInState) ByKey(k key.JobKey) etcdop.KeyT[definition.Job] {
	return j.PrefixT.Key(k.String())
}

func (j JobInState) InSink(k key.SinkKey) etcdop.PrefixT[definition.Job] {
	return j.PrefixT.Add(k.String())
}
