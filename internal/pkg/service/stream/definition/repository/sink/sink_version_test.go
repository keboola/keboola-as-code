...
func TestSinkRepository_Versions(t *testing.T) {
	...
	// -----------------------------------------------------------------------------------------------------------------
	{
		if err := repo.GetVersion(sinkKey, definition.VersionNumber(1)).Do(ctx).Err(); assert.Error(t, err) {
	...
	// -----------------------------------------------------------------------------------------------------------------
	{
		result, err := repo.GetVersion(sinkKey, version2.VersionNumber()).Do(ctx).ResultOrErr()
		assert.NoError(t, err)
	...
	// -----------------------------------------------------------------------------------------------------------------
	{
		etcdhelper.AssertKVsFromFile(t, client, "fixtures/sink_version_test_snapshot_001.txt", ignoredEtcdKeys)
	}
}
