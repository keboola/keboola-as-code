...
func TestRepository_Source(t *testing.T) {
	...
	// -----------------------------------------------------------------------------------------------------------------
	{
		...
		if err := sourceRepo.Create(&source1, clk.Now(), "Create description").Do(ctx).Err(); assert.Error(t, err) {
	...
	// -----------------------------------------------------------------------------------------------------------------
	{
		...
		require.NoError(t, branchRepo.Create(&branch1, clk.Now()).Do(ctx).Err())
		branch2 := test.NewBranch(sourceKey2.BranchKey)
		require.NoError(t, branchRepo.Create(&branch2, clk.Now()).Do(ctx).Err())
	}
	...
	// -----------------------------------------------------------------------------------------------------------------
	{
		...
		result1, err := sourceRepo.Create(&source1, clk.Now(), "Create description").Do(ctx).ResultOrErr()
		...
		result2, err := sourceRepo.Create(&source2, clk.Now(), "Create description").Do(ctx).ResultOrErr()
		require.NoError(t, err)
	...
	{
		// Modify name
		result, err := sourceRepo.Update(sourceKey1, clk.Now(), "Update description", func(v definition.Source) (definition.Source, error) {
	...
	// -----------------------------------------------------------------------------------------------------------------
	{
		err := sourceRepo.Update(nonExistentSourceKey, clk.Now(), "Update description", func(v definition.Source) (definition.Source, error) {
			v.Name = "Modified Name"
			return v, nil
		}).Do(ctx).Err()
	...
	// -----------------------------------------------------------------------------------------------------------------
	{
		assert.NoError(t, sourceRepo.SoftDelete(sourceKey1, clk.Now()).Do(ctx).Err())
	}
	{
		// Get - not found
		if err := sourceRepo.Get(sourceKey1).Do(ctx).Err(); assert.Error(t, err) {
	...
	// -----------------------------------------------------------------------------------------------------------------
	if err := sourceRepo.SoftDelete(sourceKey1, clk.Now()).Do(ctx).Err(); assert.Error(t, err) {
		assert.Equal(t, `source "my-source-1" not found in the branch`, err.Error())
		serviceErrors.AssertErrorStatusCode(t, http.StatusNotFound, err)
	}
	...
	{
		// Undelete
		result, err := sourceRepo.Undelete(sourceKey1, clk.Now()).Do(ctx).ResultOrErr()
		require.NoError(t, err)
		assert.Equal(t, "Modified Name", result.Name)
	...
	// Undelete - not found
	// -----------------------------------------------------------------------------------------------------------------
	if err := sourceRepo.Undelete(sourceKey1, clk.Now()).Do(ctx).Err(); assert.Error(t, err) {
		assert.Equal(t, `deleted source "my-source-1" not found in the branch`, err.Error())
		serviceErrors.AssertErrorStatusCode(t, http.StatusNotFound, err)
	}
	...
	}
	{
		//  Re-create
		source1 := test.NewSource(sourceKey1)
		assert.NoError(t, sourceRepo.Create(&source1, clk.Now(), "Re-create").Do(ctx).Err())
