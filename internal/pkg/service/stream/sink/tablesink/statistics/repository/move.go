...
// This operation should not be used separately but atomically together with changing the slice state.
func (r *Repository) MoveAll(parentKey fmt.Stringer, from, to storage.Level, transform ...TransformFn) *op.AtomicOp[statistics.Value] {
	if from == to {
	...
	if k, ok := parentKey.(storage.SliceKey); ok {
		// Get
