...
func TestRepository_FileAndSliceStateTransitions(t *testing.T) {
	...
	// -----------------------------------------------------------------------------------------------------------------
	{
		...
		sink.Table.Config.Storage = test.SinkStorageConfig(3, []string{"default"})
	...
	// -----------------------------------------------------------------------------------------------------------------
	{
		err := sliceRepo.StateTransition(clk.Now(), sliceKey1, storage.SliceWriting, storage.SliceUploading).Do(ctx).Err()
		require.Error(t, err)
	...
	// -----------------------------------------------------------------------------------------------------------------
	{
		err := fileRepo.StateTransition(clk.Now(), file.FileKey, storage.FileWriting, storage.FileImporting).Do(ctx).Err()
		require.Error(t, err)
	...
	// -----------------------------------------------------------------------------------------------------------------
	{
		err := fileRepo.StateTransition(clk.Now(), file.FileKey, storage.FileClosing, storage.FileImporting).Do(ctx).Err()
		require.Error(t, err)
...
`), err.Error())
	...
	// -----------------------------------------------------------------------------------------------------------------
	{
		require.NoError(t, sliceRepo.StateTransition(clk.Now(), sliceKey3, storage.SliceClosing, storage.SliceUploading).Do(ctx).Err())
...
`), err.Error())
	...
	// -----------------------------------------------------------------------------------------------------------------
	{
		require.NoError(t, sliceRepo.StateTransition(clk.Now(), sliceKey3, storage.SliceUploading, storage.SliceUploaded).Do(ctx).Err())
		slice1KV, err := sliceRepo.Get(sliceKey3).Do(ctx).ResultOrErr()
		require.NoError(t, err)
		assert.Equal(t, storage.SliceUploaded, slice1KV.State)
	}
