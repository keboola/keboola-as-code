...
type SlicePipeline struct {
	...
	slice       *SliceData
	onClose     func(context.Context, storage.SliceKey)
...
func NewSlicePipeline(ctx context.Context, logger log.Logger, connections *connection.Manager, encoding *encoding.Manager, ready *readyNotifier, slice *SliceData, onClose func(context.Context, storage.SliceKey)) *SlicePipeline {
	p := &SlicePipeline{
...
func (p *SlicePipeline) tryOpen() error {
	...
	remoteFile, err := rpc.OpenNetworkFile(ctx, p.logger, p.connections.NodeID(), conn, p.slice.SliceKey, p.slice.LocalStorage, onServerTermination)
	if err != nil {
