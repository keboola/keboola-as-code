...
func TestNetworkFile(t *testing.T) {
	...
	file, sourceNode, _ := openNetworkFile(t, ctx, etcdCfg, "source", slice.SliceKey, slice.LocalStorage)
...
func TestNetworkFile_ClosedFile(t *testing.T) {
	t.Parallel()
	...
	// Open file on a source node
	file, sourceNode, connection := openNetworkFile(t, ctx, etcdCfg, "source", slice.SliceKey, slice.LocalStorage)
	assert.True(t, file.IsReady())
	...
	// Close connection (in practice it can get closed by MirrorMap sync)
...
func openNetworkFile(t *testing.T, ctx context.Context, etcdCfg etcdclient.Config, sourceNodeID string, sliceKey model.SliceKey, slice localModel.Slice) (encoding.NetworkOutput, dependencies.Mocked, *transport.ClientConnection) {
	t.Helper()
