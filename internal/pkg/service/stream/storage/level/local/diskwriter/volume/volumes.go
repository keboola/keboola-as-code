...
func OpenVolumes(ctx context.Context, d dependencies, nodeID, volumesPath string, config diskwriter.Config) (v *Volumes, err error) {
	v = &Volumes{
		clock:  d.Clock(),
	...
	v.collection, err = opener.OpenVolumes(ctx, v.logger, nodeID, volumesPath, func(spec volume.Spec) (*Volume, error) {
		return Open(ctx, v.logger, v.clock, v.events, config, spec)
	})
	if err != nil {
		return nil, err
	}
