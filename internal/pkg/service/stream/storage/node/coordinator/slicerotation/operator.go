	...
	// Mirror slices
	{
		o.slices = etcdop.SetupMirrorMap[model.Slice, model.SliceKey, *sliceData](
			...
			WithFilter(func(event etcdop.WatchEvent[model.Slice]) bool {
				...
				if err != nil {
					o.logger.Warnf(ctx, "cannot check if the node is owner of the slices based on source: %s", err)
					return false
				}
...
func (o *operator) rotateSlice(ctx context.Context, slice *sliceData) {
	...
	if err == nil {
		compressedSize, err := safecast.Convert[int64](stats.Total.CompressedSize)
		if err != nil {
		...
		uncompressedSize, err := safecast.Convert[int64](stats.Total.UncompressedSize)
		if err != nil {
