...
func TestSliceUpload(t *testing.T) {
	...
	volumesPath := "/tmp/TestSliceUpload123"
...
`)
	...
	for _, slice := range slices {
		...
		fmt.Println(slice.LocalStorage.DirName(volumePath1))
	...
	// Trigger check - compressed size trigger
	triggerCheck(t, true, `
{"level":"info","message":"rotating slice for upload: size threshold met, compressed size: 5.0 MB, threshold: 5.0 MB","component":"storage.node.operator.slice.upload"}
`)
	...
	// Shutdown
	//deps.Process().Shutdown(ctx, errors.New("bye bye"))
	//deps.Process().WaitForShutdown()
