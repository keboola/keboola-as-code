...
func (r *FileRepository) ListRecentIn(parentKey fmt.Stringer, limit int) iterator.DefinitionT[model.File] {
	return r.schema.AllLevels().InObject(parentKey).
		GetAll(r.client, iterator.WithSort(etcd.SortDescend), iterator.WithLimit(limit))
}
