...
func TestNamingPathsStorage(t *testing.T) {
	...
	err := s.Attach(BranchKey{BranchId: 789}, NewAbsPath("", "my-branch-456"))
	assert.Error(t, err)
	...
	// Re-use path
	assert.NoError(t, s.Attach(BranchKey{BranchId: 789}, NewAbsPath("", "my-branch-456")))
	assert.Len(t, s.byPath, 2)
	assert.Len(t, s.byKey, 2)
}
