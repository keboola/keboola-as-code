...
func (u *uow) Save(object model.Object, changedFields model.ChangedFields) {
	_, exists := u.state.Get(object.Key())
	u.
		workersFor(object.Level()).
		AddWorker(func() error {
			// Validate
			if err := validator.Validate(u.ctx, object); err != nil {
				return utils.PrefixError(fmt.Sprintf(`%s is invalid`, object.String()), err)
			}

			// Get path
			path, err := u.state.namingGenerator.GetOrGenerate(object)
			if err != nil {
				return err
			}

			// Invoke mapper
			localObject := deepcopy.Copy(object).(model.Object)
			recipe := model.NewLocalSaveRecipe(path, localObject, changedFields)
			if err := u.state.mapper.MapBeforeLocalSave(recipe); err != nil {
				return err
			}

			// Save
			if err := u.state.manager.SaveObject(object, recipe, changedFields); err != nil {
				return err
			}
...
func (u *uow) Delete(object model.Key) {
	u.
		workersFor(object.Level()).
		AddWorker(func() error {
			if err := u.state.manager.deleteObject(manifest); err != nil {
				return err
			}
			u.Remove(object)
