...
func (m *coreFilesMapper) loadMetaFile(recipe *model.LocalLoadRecipe) error {
	_, _, err := recipe.Files.
		Load(m.state.NamingGenerator().MetaFilePath(recipe.Path)).
		SetDescription(recipe.Object.Kind().Name+" metadata").
...
func (m *coreFilesMapper) loadConfigFile(recipe *model.LocalLoadRecipe) error {
	_, _, err := recipe.Files.
		Load(m.state.NamingGenerator().ConfigFilePath(recipe.Path)).
		SetDescription(recipe.Object.Kind().Name).
		AddTag(model.FileTypeJson).
...
func (m *coreFilesMapper) loadDescriptionFile(recipe *model.LocalLoadRecipe) error {
	_, _, err := recipe.Files.
		Load(m.state.NamingGenerator().DescriptionFilePath(recipe.Path)).
		SetDescription(recipe.Object.Kind().Name+" description").
