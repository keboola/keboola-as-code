...
func (m *orchestratorLocalMapper) AfterLocalOperation(state *local.State, changes *model.Changes) error {
	...
	for _, object := range changes.Loaded() {
		if ok, err := m.isOrchestrator(object.Key()); err != nil {
			...
			if err := m.onLocalLoad(state, object.(*model.Config)); err != nil {
				errs.Append(err)
			}
		}
	}
...
func (m *orchestratorLocalMapper) onLocalLoad(state *local.State, orchestrator *model.Config) error {
	relatedPaths, err := state.GetRelatedPaths(orchestrator)
	if err != nil {
		return err
	}

	parentCtx, err := local.NewLoadContext(context.Background(), state.FileLoader(), state, orchestrator)
	if err != nil {
		return err
	}

	loadCtx := &localLoadContext{
		LoadContext:   parentCtx,
	...
	if err := loadCtx.load(); err != nil {
		return errors.PrefixError(fmt.Sprintf(`invalid orchestrator config "%s"`, loadCtx.BasePath()), err)
	}
	return nil
}
...
func (c *localLoadContext) loadPhaseConfig(phase *model.Phase, phaseDir model.AbsPath) (dependsOn []string, err error) {
	...
	file, err := c.
		...
		AddTag(local.FileKindPhaseConfig).
		ReadJsonFile()
	if err != nil {
...
func (c *localLoadContext) loadTaskConfig(task *model.Task, taskDir model.AbsPath) error {
	...
	file, err := c.
		...
		AddTag(local.FileTypeJson).
		AddTag(local.FileKindTaskConfig).
		ReadJsonFile()
	if err != nil {
