...
type WorkspaceDetails struct {
	...
	// When BaseURL is set, DBT_KBC_{TARGET}_BASE_URL / _BRANCH_ID / _WORKSPACE_ID are written.
	...
	for _, bucket := range o.Buckets {
		if bucket.LinkedProjectID != 0 {
			...
			if _, exists := linkedBucketEnvsMap[envVarName]; !exists {
				stackPrefix, _, _ := strings.Cut(o.Workspace.Database, "_")
