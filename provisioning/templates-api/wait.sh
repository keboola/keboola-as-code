...
echo "--------------------------"
if kubectl rollout status sts/templates-api-etcd --namespace "$NAMESPACE" --timeout "$KUBERNETES_ROLLOUT_WAIT"; then
  echo
  ...
  echo "Logs:"
  echo "--------------------------"
  kubectl logs --namespace "$NAMESPACE" --selector "app=templates-api-etcd" --all-containers --prefix --timestamps --tail=-1
  echo "--------------------------"
  exit 1
fi
